...
def wall(update: Update, context: CallbackContext) -> None:
    ...
    if user_input != " ":
        r = requests.post("https://godbolt.org/api/compiler/r1730/compile", params = {"options": "-Wall"} , data=user_input ,
        headers = {"Content-Type": "text/plain"})
        if r.status_code == 200:
            assembly = r.text
            update.message.reply_text(assembly,parse_mode='Markdown')
        else:
